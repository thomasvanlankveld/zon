...
1. Run the release script (from the `master` branch)
   ...
   - Options (pass after `--`): `--dry-run` (checks only), `--no-push` (tag locally but don't push), `--yes` (skip confirmation; use in CI or other non-interactive use), `--skip-tests`, `--skip-lint`. Example: `deno task release:patch -- --dry-run`
